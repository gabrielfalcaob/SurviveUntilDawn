...
Player* & Missile::player = SurviveUntilDawn::player;        // referência para o player
...
void Missile::Update()
{
    ...
    if (x > game->Width() - 50 || x < 50 || y > game->Height() - 50 || y < 50)
    {
        ...
        SurviveUntilDawn::audio->Volume(HITWALL, level * BaseVolume);
        SurviveUntilDawn::audio->Play(HITWALL);

        // adiciona explosão na cena
        SurviveUntilDawn::scene->Add(new WallHit(x,y), STATIC);
